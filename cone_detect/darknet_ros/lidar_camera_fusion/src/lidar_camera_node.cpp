...
std::string imgTopic_blue = "/usb_cam/image_raw_blue";
...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2>& in_pc2 , const ImageConstPtr& in_image, const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg, const ImageConstPtr& in_image_blue)
{
  // cout << "asdsa" << endl;
 ...
 // pcl::PointXYZI point_aux;
  ...
  {
    // if(msg->bounding_boxes[j].Class[0] == 'y')
    if(msg->bounding_boxes[j].xmax < 640){
      bbox_xmax[j] = msg->bounding_boxes[j].xmax;
      bbox_xmin[j] = msg->bounding_boxes[j].xmin;
    ...
    // if(msg->bounding_boxes[j].Class[0] == 'b')
    if(msg->bounding_boxes[j].xmax > 640){
      bbox_xmax[j] = msg->bounding_boxes[j].xmax-640;
      if (msg->bounding_boxes[j].xmin < 640)
        bbox_xmin[j] = 0;
      else
  ...
  pcOnimg_pub.publish(cv_ptr_blue->toImageMsg());
  //minimum
...
int main(int argc, char** argv)
{
  ...
  typedef sync_policies::ApproximateTime<PointCloud2, Image, darknet_ros_msgs::BoundingBoxes, Image> MySyncPolicy;
  Synchronizer<MySyncPolicy> sync(MySyncPolicy(100), pc_sub, img_sub, bbox_sub, img_sub_blue);
