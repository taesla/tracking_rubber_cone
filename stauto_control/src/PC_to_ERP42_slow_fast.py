...
def acker_callback1(msg):
    global speed, steer, brake, gear, fast_flag

    if fast_flag == False:

        speed = 1.5
        steer = -msg.data # why minus?
        brake = 0
        gear = 0 
        #print(speed)    
...
def acker_callback2(msg):
    global speed, steer, brake, gear, j, fast_flag
    
    #os.system("rosnode kill"+ "/serial_node_slow")
    if not msg.drive.steering_angle == 50000:
        fast_flag =True
        steer = -msg.drive.steering_angle # why minus?
        
        if int(msg.drive.jerk) ==0:
            j=0
            brake = 0
            speed = 5
        else:
...
if __name__ == '__main__':
    rospy.init_node('serial_node_slow')

    rospy.Subscriber("/steer",Float32,acker_callback1)
    rospy.Subscriber("/ackermann_cmd",AckermannDriveStamped,acker_callback2)
